...
Opened from Settings → 🧠 Memory tab → "Open Memory Dashboard" button,
OR directly from tray menu.

Shows:
  • Known facts about the user (editable, deletable)
  • Recent conversation exchanges
  • App time breakdown
  • Emotion history timeline
  • Peak hours
  • Nuclear "Forget Everything" option
"""
...
class MemoryDashboard:
    ...
    def _build(self):
        ...
        tk.Label(hdr, text=f"{sessions} sessions together",
                 bg=ACCENT, fg="#D0C0FF",
    ...
    def _add_fact_row(self, parent, key, value):
        ...
        e.pack(side="left", fill="x", expand=True, padx=4, pady=4)
    ...
    def _add_new_fact_section(self, parent):
        ...
        tk.Label(parent, text="Add a fact manually:",
                 bg=BG, fg=TEXT2, font=("Segoe UI", 8)).pack(
                 anchor="w", padx=16)
    ...
    def _build_apps(self, nb):
        ...
        if not sorted_apps or max_mins == 0:
            ...
            for app, mins in sorted_apps[:20]:
                ...
                bar = tk.Frame(bar_frame, bg=ACCENT, height=14)
    ...
    def _clear_apps(self):
        if messagebox.askyesno("Reset App Stats",
                               "Clear all app time data?",
                               parent=self._win):
    ...
    def _build_stats(self, nb):
        ...
        tk.Label(f, text="LLM context block (what gets injected into AI brain):",
                 bg=BG, fg=TEXT2, font=("Segoe UI", 8)).pack(
                 anchor="w", padx=12, pady=(12,2))

        ctx = self._memory.build_context_block()
        txt = tk.Text(f, bg=BG2, fg=TEXT2, font=("Segoe UI", 8),
                      relief="flat", height=5, wrap="word", state="normal",
                      padx=8, pady=6)
    ...
    def _save_and_refresh(self):
        # Write edited fact values back to memory
        for key, var in self._fact_vars.items():
            ...
            if val:
                ...
                self._memory._data["facts"].pop(key, None)
        self._memory.save()
        # Flash the header green briefly
        try:
